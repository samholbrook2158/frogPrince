...
app.post('/signup', function (req, res) {
    ...
    db.query('SELECT * FROM users WHERE username = ? OR email = ?', [username, email], function (err, results) {
        if (err) {
            ...
            db.query('INSERT INTO users (username, email, password, organization) VALUES (?, ?, ?, ?)', [username, email, password, organization], function (err, result) {
                if (err) {
                    res.render('signup', { error: 'Error occurred while processing the request.' });
                } else {
                    // If signup is successful, redirect to login page with success message
